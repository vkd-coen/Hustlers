...
/* Opponent tracking — read by behavior.c */
...
void Check_PC_Command(void)
{
    if (HAL_UART_Receive(&huart2, &pc_cmd, 1, 0) == HAL_OK)
    {
        if (pc_cmd == CTRL_C_BYTE)
        {
            ...
            {
                output_paused = 1;
                Debug_Print("\r\n--- CTRL+C: pausing LIDAR and stopping motors ---\r\n");
                RPLIDAR_Stop();
                lidar_scan_running = 0;
                Behavior_Stop();
                Motors_Stop();
                Debug_Print("--- PAUSED. Press Ctrl+C to resume. ---\r\n");
            }
            else
            {
                output_paused = 0;
                Debug_Print("\r\n--- CTRL+C: restarting LIDAR and motors ---\r\n");
                if (!RPLIDAR_Start_Scan())
                    Debug_Print("RPLIDAR scan restart failed.\r\n");
                Behavior_Start();
                Debug_Print("--- RESUMED. ---\r\n");
            }
        }
...
{
    uint8_t data = 0x00;
    HAL_I2C_Mem_Write(&hi2c1, mpu_addr, MPU6050_PWR_MGMT_1_REG,   I2C_MEMADD_SIZE_8BIT, &data, 1, 100);
    HAL_Delay(100);
    HAL_I2C_Mem_Write(&hi2c1, mpu_addr, MPU6050_GYRO_CONFIG_REG,  I2C_MEMADD_SIZE_8BIT, &data, 1, 100);
    HAL_I2C_Mem_Write(&hi2c1, mpu_addr, MPU6050_ACCEL_CONFIG_REG, I2C_MEMADD_SIZE_8BIT, &data, 1, 100);
}

HAL_StatusTypeDef MPU6050_Read_All(void)
{
    HAL_StatusTypeDef status = HAL_I2C_Mem_Read(&hi2c1, mpu_addr, MPU6050_ACCEL_XOUT_H_REG,
                                                I2C_MEMADD_SIZE_8BIT, imu_data, 14, 100);
...
void RPLIDAR_GetInfo_Test(void)
{
    ...
    if (status == HAL_OK && lidar_info_rx[0] == 0xA5 && lidar_info_rx[1] == 0x5A)
...
uint8_t RPLIDAR_Start_Scan(void)
{
    ...
    if (status != HAL_OK || lidar_desc[0] != 0xA5 || lidar_desc[1] != 0x5A) {
        lidar_scan_running = 0;
        return 0;
    }
...
uint8_t RPLIDAR_Read_Valid_Node(uint8_t *node, uint32_t timeout_ms)
{
    ...
    while ((HAL_GetTick() - start_time) < timeout_ms) {
        if (HAL_UART_Receive(&huart3, &b, 1, 2) == HAL_OK) {
            ...
            node[3] = node[4]; node[4] = b;
            uint8_t s = node[0] & 0x01;
            uint8_t si = (node[0] >> 1) & 0x01;
            uint8_t c = node[1] & 0x01;
            if ((s != si) && (c == 1)) return 1;
        }
    }
    return 0;
}
...
void RPLIDAR_Process_Node(uint8_t *node)
{
    ...
    /* Track nearest in-arena point for opponent detection */
...
int main(void)
{
  ...
  HAL_Delay(1000);
  Debug_Print("\r\nSTM32 started\r\n");
  Debug_Print("Press Ctrl+C in PuTTY to pause/resume.\r\n");
...
/* ============================================================
   The functions below are unchanged from your current main.c —
   SystemClock_Config, MX_I2C1_Init, MX_USART2_UART_Init,
   MX_USART3_UART_Init, MX_GPIO_Init, Error_Handler.
   Keep your existing versions.

   You must ADD MX_TIM2_Init and MX_TIM3_Init — these are
   auto-generated when you configure TIM2 and TIM3 in CubeMX
   (see notes in chat for the exact pin assignments).
   ============================================================ */
